...
%% CL CD and CY construction
  CL = CL_0 + (CL_alpha * alpha + CL_eq*( c/(2*V) ) * q + CL_de * de)*0.01;
  CD = CD_0 + ((CL^2)/(pi*AR*e))*0.1;
  CY = 0.5+(CY_beta*beta+CY_dr*dr);
%% Forces
...
%% Cl Cp Cn - Moments Coefficients
  Cl =Cl_beta*beta+ Cl_alpha_beta*alpha*beta+(Cl_p*(b/(2*V))*p+Cl_r *(b/(2*V))*r + Cl_da*da+Cl_dr*dr)*0.1;
  Cm = Cm_0+Cm_alpha*alpha+ Cm_beta*beta+( Cm_eq*(c/(2*V))*q+Cm_de*de)*0.1;
  Cn=Cn_beta*beta+(Cn_p*(b/(2*V))*p+Cn_r*(b/(2*V))*r+Cn_da*da+Cn_dr*dr)*0.01;
%% Moments
